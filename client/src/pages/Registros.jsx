              ...
              {paginated.map((r) => (
                <div key={r.id} className="p-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    {(r.placaTop || r.status === 'Rastreador') && (
                      <button
                        onClick={() => handleDoc(r, r.status === 'Rastreador' ? 0 : 1)}
                        disabled={pdfLoading === `${r.id}-${r.status === 'Rastreador' ? 0 : 1}`}
                      ...
                      >
                        {pdfLoading === `${r.id}-${r.status === 'Rastreador' ? 0 : 1}` ? <svg className="animate-spin w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg> : <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>}
                        TOP
                      </button>
                    )}
                  </div>
